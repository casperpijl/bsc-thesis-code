...
The full prompt used in the experiment, adapted from the MixSolDB
extraction prompt (Voinea et al.) to output JSON. The ablation prompts
are made by removing parts of this one.
...
- For mixtures, extract and record:
    - Each solvent's name.
    - The mole fraction, volume fraction, or mass fraction of each solvent in the mixture. If a fraction is not explicitly given, it may be inferred from context if feasible, otherwise skip it. Always prefer mole fraction if available, and only report one type of fraction per solvent, placed under the `concentration_1` and `concentration_2` columns of `data`.
