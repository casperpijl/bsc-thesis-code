# Cumulative prompt (qwen3.5-9b, level 8, reasoning on)

## System prompt

For mixtures, track solvent combinations and their respective mole, volume, or mass fractions.
...
- For mixtures, extract and record:
    - Each solvent's name.
    - The mole fraction, volume fraction, or mass fraction of each solvent in the mixture. If a fraction is not explicitly given, it may be inferred from context if feasible, placed under the `concentration_1` and `concentration_2` columns of `data`.
